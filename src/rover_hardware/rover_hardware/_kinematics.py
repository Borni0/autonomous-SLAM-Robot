...
def twist_to_wheel_pcts(
    ...
    half = wheel_base / 2.0
    v_L = v_mps - w_radps * half
    v_R = v_mps + w_radps * half
...
def integrate_pose(
    ...
    Mirrors Odometry::update in
    ~/AutonomousNavigationRobot/src/odometry.cpp so dead-reckoning here
    matches the firmware's pose estimates bit-for-bit (when given the
    same wheel velocities).
    """
    import math
